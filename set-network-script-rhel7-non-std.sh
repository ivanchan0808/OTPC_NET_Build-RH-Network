...
get_bond_list() {
    ...
    done < <(grep -il "Device=Bond" "$config_path"/ifcfg-* 2> /dev/null)
...
get_bond_nic_list() {
    ...
    done < <(grep -il "TYPE=Ethernet" "$config_path"/ifcfg-* | xargs grep -l MASTER=$bond 2> /dev/null)
...
set_new_nic() {
    ...
    # Case 4: eth (RHEL7)
    elif [[ ${nic%%[0-9]*} == "eth" ]]; then
        local num=${nic//[!0-9]/}
        local new_num=$((num + 1))
        local prefix=${nic%%[0-9]*}
        new_nic="${prefix}${new_num}"

    # Case 5: Test ENV VM NIC on Linux e.g. ens192 -> ens 161; ens224 -> ens256
...
##### Add on 22-Aug-2025 ( Single NIC handling)
set_special_eth_file(){
    local src_nic=$1
    local new_nic=$2
    local temp_name=$2"-temp"
    local cfg_dir=$3
    local srcfile=""
    local newfile=""

    ##### New nic -> Temp Name
    files=$(find $3 -maxdepth 1 -name "ifcfg-${new_nic}*")
    for f in $files; do
        srcfile="$f"
        newfile="${f/${new_nic}/${temp_name}}"   # Replace in filename

        echo "Renaming: $srcfile -> $newfile"
        mv "$srcfile" "$newfile"   

    done

    files=$(find $3 -maxdepth 1 -name "route-${new_nic}*")
    for f in $files; do
        srcfile="$f"
        newfile="${f/${new_nic}/${temp_name}}"   # Replace in filename
    ...
    ##### Old nic -> New nic
    # Find all ifcfg files for old NIC (including VLANs)
    files=$(find $3 -maxdepth 1 -name "ifcfg-${src_nic}*")
    for f in $files; do
        srcfile="$f"
        newfile="${f/${src_nic}/${new_nic}}"   # Replace in filename

        echo "Renaming: $srcfile -> $newfile"
        mv "$srcfile" "$newfile"

        # Replace DEVICE= line and any mention of OLD_NIC inside
        sed -i "s/${src_nic}/${new_nic}/g" "$newfile"
    done

    files=$(find $3 -maxdepth 1 -name "route-${src_nic}*")
    for f in $files; do
        srcfile="$f"
        newfile="${f/${src_nic}/${new_nic}}"   # Replace in filename
    ...
    ##### Temp name -> Old NIC
    files=$(find $3 -maxdepth 1 -name "ifcfg-${temp_name}*")

    for f in $files; do
        srcfile="$f"
        newfile="${f/${temp_name}/${src_nic}}"   # Replace in filename

        echo "Renaming: $srcfile -> $newfile"
        mv "$srcfile" "$newfile"   

        # Replace DEVICE= line and any mention of OLD_NIC inside
        sed -i "s/${new_nic}/${src_nic}/g" "$newfile"
    done

    files=$(find $3 -maxdepth 1 -name "route-${temp_name}*")
    for f in $files; do
        srcfile="$f"
        newfile="${f/${temp_name}/${src_nic}}"   # Replace in filename
...
if [[ ! -z "$1" ]]; then
	...
	SERVER=`find $NS_CONFIG_PREFIX -name hostname|xargs cat`
	...
	#NS_LOG_FILE=`find ${NS_CONFIG_PREFIX} -name network_info_*.log`
        NS_LOG_FILE="/root/RHEL7_20250802/ip_info_output_${2}.txt"
	...
	LAST_RUN_FOLDER="${NS_NEW_CONFIG_PATH}last_run/"
        ...
        if grep -i "Device=Bond" "${NS_CONFIG_PATH}ifcfg-bond"* ; then
...
##### Debug use    	
#bond_array=($(get_bond_nic_list $MAIN_COPY_PATH $bond))
#echo "NICs in bond:"
#for nic in "${bond_array[@]}"; do
#    echo " - $nic"
#done
...
##### Add on 22-Aug-2025 Special handling cases
if [[ $SERVER == "pomdclt101" ]]; then

	set_special_eth_file "ens1f1" "ens1f0" $APPROACH_BA_PATH
	set_special_eth_file "ens4f1" "ens4f0" $APPROACH_BA_PATH
	
	set_special_eth_file "ens1f1" "ens1f0" $APPROACH_BB_PATH
	set_special_eth_file "ens4f1" "ens4f0" $APPROACH_BB_PATH

fi

if [[ $SERVER == "pspdapp002" || $SERVER == "somdclt101" || $SERVER == "sspdapp002" ]]; then
        set_special_eth_file "ens1f0" "ens1f1" $APPROACH_BA_PATH
        set_special_eth_file "ens4f0" "ens4f1" $APPROACH_BA_PATH

        set_special_eth_file "ens1f0" "ens1f1" $APPROACH_BB_PATH
        set_special_eth_file "ens4f0" "ens4f1" $APPROACH_BB_PATH
